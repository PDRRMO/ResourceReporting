      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md mx-4 overflow-hidden animate-in zoom-in-95 duration-200">
        ...
        <div className="p-6 space-y-3">
          <p className="text-sm text-slate-500 mb-4">
            Select the current status for this resource:
          </p>
          {statuses.map((status) => {
      ...
      <div className="p-6 space-y-6 overflow-y-auto h-[calc(100%-80px)]">
        ...
        <div className="space-y-3">
          <button
            ...
            className="w-full py-3 bg-[#2563eb] text-white rounded-xl font-semibold hover:bg-[#1d4ed8] transition-colors shadow-lg shadow-blue-500/25"
          >
            Update Status
          </button>
    ...
    if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(
        (position) => {
          ...
          showSuccess("Location Found", "Map centered on your current location");
        },
        (error) => {
          console.error("Error getting location:", error);
          showError("Location Error", "Unable to get your location. Please enable location services.");
        }
      );
    } else {
  ...
  return (
    <div className="w-screen h-screen relative overflow-hidden bg-slate-900">
      ...
      <div className="map-control-panel absolute top-25 right-6 z-40 flex flex-col gap-3 md:top-5">
        ...
        >
          <LayoutDashboard size={22} className="text-[#2563eb] group-hover:scale-110 transition-transform" />
        </Link>

        {/* Add New Resource Button */}
        <Link
          href="/upload"
          className="w-12 h-12 bg-[#2563eb] backdrop-blur-xl rounded-2xl shadow-xl shadow-blue-500/30 border border-blue-500/20 flex items-center justify-center hover:bg-[#1d4ed8] transition-all group"
        ...
        <div className="flex items-center justify-center">
          <TourButton tourName="map" className="w-12 h-12 bg-white/90 backdrop-blur-xl rounded-2xl shadow-xl border border-white/20 flex items-center justify-center hover:bg-white transition-all" />
        </div>

        {/* Filter Settings */}
        <button 
          onClick={() => setIsFilterOpen(true)}
          className="map-filter-btn w-12 h-12 bg-white/90 backdrop-blur-xl rounded-2xl shadow-xl border border-white/20 flex items-center justify-center hover:bg-white transition-all group relative"
        >
      ...
      <StatusUpdateModal
        ...
        currentStatus={selectedMarker?.status || "ready"}
